...
이 폴더는 `experiments/reference_1_section7_2`의 Python 실험을 MATLAB 코드만으로 다시 구현한 버전입니다. Python `src.common`을 호출하지 않고, 모델 생성, random projection, random sampling, non-random baseline, metric 계산, CSV/PNG 저장을 `+sec72/Common.m` 안에 독립적으로 넣었습니다.
