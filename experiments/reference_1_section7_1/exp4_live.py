...
"""Section 7.1 Experiment 4 (n variation with alpha_n=2/sqrt(n)) using shared common utilities."""
...
def summarize(df_raw: pd.DataFrame) -> pd.DataFrame:
    ...
    alpha_summary = df_raw.groupby(["n", "method"], as_index=False).agg(alpha_n_mean=("alpha_n", "mean"))
...
def main():
    parser = argparse.ArgumentParser(description="Section 7.1 Experiment 4")
    ...
    parser.add_argument(
        "--outdir",
        type=str,
        default="experiments/section7_1/results/exp4_section7_1_results",
    ...
    raw_csv = outdir / "exp4_raw_per_rep.csv"
    sum_csv = outdir / "exp4_summary_mean_std.csv"
    ...
    if args.detailed_timing:
        df_timing_raw = extract_timing_breakdown(df_raw, id_cols=["n", "alpha_n", "rep", "method"])
        df_timing_sum = summarize_timing_breakdown(df_timing_raw, group_cols=["n", "alpha_n"])
        timing_raw_csv = outdir / "exp4_timing_breakdown_raw.csv"
        timing_sum_csv = outdir / "exp4_timing_breakdown_summary.csv"
        df_timing_raw.to_csv(timing_raw_csv, index=False)
        df_timing_sum.to_csv(timing_sum_csv, index=False)

    if not args.no_plot:
        plot_metric_panels(df_sum, x_col="n", out_png=outdir / "figure4_like_metrics.png")
        plot_runtime(df_sum, x_col="n", out_png=outdir / "figure4_like_runtime.png")
    ...
    if not args.no_plot:
        print(f"Metrics PNG : {(outdir / 'figure4_like_metrics.png').resolve()}")
        print(f"Runtime PNG : {(outdir / 'figure4_like_runtime.png').resolve()}")
