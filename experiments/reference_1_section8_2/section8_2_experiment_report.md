...
| 방법 | 측정 시간 |
|---|---|
| Random Projection | random projection 기반 eigenvector 계산 전체 시간 |
...
각 값은 20회 반복의 median time이다. 단위는 초다.
...
따라서 Random Projection은 이론적으로 큰 matrix의 spectral subspace를 빠르게 근사하는 방법이지만, 실제 runtime은 sparse matrix multiplication 구현과 하드웨어 환경에 크게 좌우된다.
...
5. DBLP의 Random Sampling은 논문과 비교적 근접하다.
6. Internet의 Random Sampling은 sampling 제외 시간은 논문과 근접하지만 sampling 포함 시간은 느리다.
7. partial_eigen은 현재 Python/scipy proxy가 논문 baseline보다 훨씬 빠르게 나와, 논문값과 절대시간 비교가 어렵다.
8. 따라서 8.2는 실험 구조와 timing 정의는 재현되었지만, 절대 runtime은 구현체와 환경 차이 때문에 부분 재현으로 보는 것이 타당하다.
