...
- **Section 8.1은 부분적으로 재현되었다.**
  방법별 순위 경향은 대체로 유사하지만, 절대 성능값은 지표별로 차이가 크다.
...
- Section 8.1은 순위 경향은 유사하지만 절대값은 다를 수 있음
...
논문 Table 2(a, arXiv v3)와 현재 로컬 결과를 직접 대조하면 다음과 같다.

| 방법 | F1 (논문 → 재현) | NMI (논문 → 재현) | ARI (논문 → 재현) |
|---|---|---|---|
| Random Projection | 0.165(0.007) → 0.315(0.018) (`+90.9%`) | 0.558(0.006) → 0.548(0.010) (`-1.8%`) | 0.100(0.009) → 0.091(0.008) (`-9.3%`) |
| Random Sampling (`p=0.7`) | 0.126(0.007) → 0.271(0.014) (`+114.9%`) | 0.417(0.010) → 0.505(0.011) (`+21.1%`) | 0.059(0.008) → 0.079(0.008) (`+34.7%`) |
| Random Sampling (`p=0.8`) | 0.131(0.005) → 0.287(0.016) (`+118.8%`) | 0.436(0.010) → 0.526(0.009) (`+20.6%`) | 0.064(0.006) → 0.091(0.009) (`+41.4%`) |
| Non-random | 0.154(0.006) → 0.325(0.016) (`+111.1%`) | 0.571(0.005) → 0.556(0.008) (`-2.7%`) | 0.088(0.007) → 0.092(0.006) (`+4.8%`) |

정리하면:

- F1은 모든 방법에서 논문 대비 크게 높다.
- NMI/ARI는 Random Projection, Non-random은 비교적 근접하지만, Random Sampling에서는 차이가 크다.
- 즉, “사실상 동일”로 보기 어렵고 지표별 편차가 분명하다.

### 5.4 해석

Section 8.1은 **부분 재현**으로 보는 것이 적절하다.

이유는 다음과 같다.

- 방법별 순위 경향(예: Non-random 상위, `p=0.8 > p=0.7`)은 대체로 유사하다.
- 그러나 절대값 기준으로는 특히 F1에서 큰 격차가 나타난다.
- Random Sampling의 NMI/ARI 역시 논문 대비 유의미한 편차가 있다.

### 5.5 Section 8.1 결론

> Section 8.1 European Email 실험은 방법별 순위 경향은 대체로 유사하지만, 절대 성능값은 지표별로 차이가 커서 부분 재현으로 해석하는 것이 타당하다.
...
## 8. 왜 Section 8.1과 Section 8.2 모두 차이가 나는가
...
- 다만 데이터 전처리(LCC 처리), 라벨 정렬 방식, 반복 시드, 구현 세부 설정에 영향을 받을 수 있다.
- 따라서 방법별 순위는 유사하게 재현될 수 있어도 절대값은 다르게 나올 수 있다.
...
- 방법별 우열 관계는 대체로 유사하다.
- 그러나 절대 성능값(특히 F1)은 논문 대비 차이가 크다.
- 따라서 수치 일치 관점에서는 강한 재현으로 보기 어렵다.
...
> Section 8.1은 방법별 순위 경향은 대체로 유사하게 재현되었으나, Table 2(a)의 절대 성능값은 지표별로 차이가 커 부분 재현으로 해석하는 것이 타당하다. 반면 Section 8.2는 데이터 규모, target rank, timing protocol 측면에서는 논문과 정합적으로 재현되었으나, 절대 실행시간은 부분적으로만 일치하였다. 특히 논문에서 사용한 R 기반 `partial_eigen` 구현을 Python `SciPy` 기반 proxy로 대체하였기 때문에 baseline runtime 특성이 달라졌고, 이로 인해 Table 4의 절대시간을 1:1로 동일하게 재현하지는 못했다.

## 11. 최종 결론

- **Section 8.1:** 순위 경향은 유사하지만 절대값은 차이가 큰 부분 재현 실험
