...
"""Section 7.2 Models 1-3 reproduction using shared common utilities."""
...
def _build_B_model12(rng: np.random.Generator) -> np.ndarray:
    K = 3
    B = np.zeros((K, K), dtype=float)
    for i in range(K):
        B[i, i] = rng.uniform(0.2, 0.3)
    for i in range(K):
        for j in range(i + 1, K):
            v = rng.uniform(0.01, 0.1)
            B[i, j] = v
            B[j, i] = v
    return B


def _build_B_model3() -> np.ndarray:
    C = np.array(
        [
...
def generate_model123_instance(
    model_id: int,
    n: int,
    rng: np.random.Generator,
):
    if model_id not in {1, 2, 3}:
        raise ValueError("model_id must be one of {1,2,3}.")

    if model_id == 2:
        sizes = _sizes_from_proportions(n, np.array([1.0 / 6.0, 1.0 / 2.0, 1.0 / 3.0]))
    else:
        sizes = _sizes_from_proportions(n, np.ones(3) / 3.0)

    y_true = _labels_from_sizes(sizes, rng)
    B_true = _build_B_model12(rng) if model_id in {1, 2} else _build_B_model3()
    P = B_true[y_true][:, y_true]
    np.fill_diagonal(P, 0.0)
    A = _sample_symmetric_adjacency(P, rng)
    return A, P, B_true, y_true


def run_experiment72_models123(
    cfg: Exp72Models123Config,
    ...
    for model_id in cfg.model_ids:
        for n in cfg.n_values:
            for rep in range(1, cfg.reps + 1):
                ...
                t0 = perf_counter()
                A, P, B_true, y_true = generate_model123_instance(model_id=model_id, n=n, rng=rng)
                t_instance = perf_counter() - t0
                Theta_true = np.eye(cfg.K)[y_true]
                K_prime = cfg.K_prime_rankdef if model_id == 3 else cfg.K_prime_fullrank
                ...
                if detailed_timing:
                    ...
                    t0 = perf_counter()
                    Ahat_rp, y_rp = run_random_projection(A, cfg.K, K_prime, cfg.r, cfg.q, rng)
                ...
                if detailed_timing:
                    ...
                    t0 = perf_counter()
                    Ahat_rs, y_rs = run_random_sampling(A, cfg.K, K_prime, cfg.p, rng)
                ...
                if detailed_timing:
                    ...
                    t0 = perf_counter()
                    Ahat_nr, y_nr = run_non_random(A, cfg.K, K_prime, rng)
...
def plot_models123_metrics(summary: pd.DataFrame, out_png: Path):
    ...
    fig, axes = plt.subplots(3, 3, figsize=(15.5, 11.0), sharex=True)
...
def plot_models123_runtime(summary: pd.DataFrame, out_png: Path):
    models = [1, 2, 3]
...
def main():
    parser = argparse.ArgumentParser(description="Section 7.2 Models 1-3")
    ...
    parser.add_argument(
        "--outdir",
        type=str,
        default="experiments/reference_1_section7_2/results/exp72_models123_paper_aligned_live",
    ...
    print("Running Section 7.2 Models 1-3...")
    df_raw = run_experiment72_models123(
        cfg,
    ...
    raw_csv = outdir / "sec72_models123_raw_per_rep.csv"
    sum_csv = outdir / "sec72_models123_summary_mean_std.csv"
    ...
    if args.detailed_timing:
        ...
        timing_raw_csv = outdir / "sec72_models123_timing_breakdown_raw.csv"
        timing_sum_csv = outdir / "sec72_models123_timing_breakdown_summary.csv"
        df_timing_raw.to_csv(timing_raw_csv, index=False)
        df_timing_sum.to_csv(timing_sum_csv, index=False)

    if not args.no_plot:
        plot_models123_metrics(df_sum, outdir / "sec72_models123_metrics_figure5_like.png")
        plot_models123_runtime(df_sum, outdir / "sec72_models123_runtime.png")
    ...
    if not args.no_plot:
        print(f"Metrics PNG : {(outdir / 'sec72_models123_metrics_figure5_like.png').resolve()}")
        print(f"Runtime PNG : {(outdir / 'sec72_models123_runtime.png').resolve()}")
