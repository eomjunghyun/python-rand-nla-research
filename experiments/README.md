...
- `메인 실험/`: 비균일 HSBM의 `n` 변화, 큰 `n` 변화, `K` 변화 실험
- `균일 HSBM 실험/`: planted `d`-uniform HSBM의 `n` 변화 실험
- `nonuniform_hsbm_scaling/`: 초기 비균일 HSBM 스케일링 작업 폴더. 현재 주요 노트북은 `메인 실험/`에서 관리
...
- `reference_2_exp3/`: Figure 6~12 재현. Hyperedge composition과 Gini 분석 포함
