...
"""Section 8.1 European Email 정확도 실험의 target rank 30 변형.
...
Rank-30 변형:
- spectral embedding에는 고유벡터/특징 30개를 사용한다.
- KMeans의 군집 개수는 정답 class 수를 그대로 사용한다.
"""
...
def run_experiment(cfg: Exp81Config, A, y_true: np.ndarray, K: int):
    # target_rank는 spectral embedding 차원만 조절한다.
    # 출력 군집 수는 K로 유지되므로 정답에 없는 label class를 추가로 만들지 않는다.
    Kp = int(cfg.target_rank)
    if Kp <= 0:
        raise ValueError(f"target_rank must be positive, got {Kp}.")
    if Kp >= A.shape[0]:
...
def write_table2a_markdown(tbl: pd.DataFrame, out_md: Path, reps: int, target_rank: int):
    lines = []
...
def main():
    ...
    parser.add_argument("--target-rank", type=int, default=30, help="spectral embedding dimension")
    ...
    print("Running Section 8.1 European email experiment (target_rank=30 variant)...")
    A, y_true, K, stats = load_email_eu_core_lcc(cfg.edge_path, cfg.label_path)
    print(
        f"LCC stats: nodes={stats['lcc_nodes']}, edges={stats['lcc_edges']}, "
        f"K={stats['num_classes']}, target_rank={cfg.target_rank}"
    )
    ...
    write_table2a_markdown(table2a, table_md, cfg.reps, cfg.target_rank)
